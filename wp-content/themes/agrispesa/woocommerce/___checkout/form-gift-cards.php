...
if ( 'yes' !== $direct_display ) :
	...
	<div class="ywgc_have_code">
		...
		if ( get_option( 'ywgc_icon_text_before_gc_form', 'no' ) === 'yes' ) {
			...
			$icon = '<span class="icon-heart gift-card-icon-heart"></span>'; }

		wc_print_notice( $icon . get_option( 'ywgc_text_before_gc_form', esc_html__( 'Hai ricevuto una Gift Card?', 'yith-woocommerce-gift-cards' ) ) . ' <a href="#" class="ywgc-show-giftcard">' . get_option( 'ywgc_link_text_before_gc_form', esc_html__( 'Usalo qui!', 'yith-woocommerce-gift-cards' ) ) . '</a>', 'notice' );
		?>
		...
		<div class="coupon-form">
			<div class="coupon-form--sx">
				<input type="text" name="gift_card_code" class="input-text"
					placeholder="<?php echo esc_attr( apply_filters( 'ywgc_checkout_box_placeholder', _x( 'Codice Gift Card', 'Applica', 'yith-woocommerce-gift-cards' ) ) ); ?>"
					id="giftcard_code" value="" />
			</div>
			<div class="coupon-form--dx">
				<button type="submit" class="posh-button posh-button-primary posh-button-small ywgc_apply_gift_card_button" name="ywgc_apply_gift_card" value="<?php echo wp_kses( get_option( 'ywgc_apply_gift_card_button_text', esc_html__( 'Applica', 'yith-woocommerce-gift-cards' ) ), 'post' ); ?>"><?php echo wp_kses( get_option( 'ywgc_apply_gift_card_button_text', esc_html__( 'Applica', 'yith-woocommerce-gift-cards' ) ), 'post' ); ?></button>
