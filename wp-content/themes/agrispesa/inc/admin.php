<?php
	...
	function give_user_subscription($product, $user, $row) {
		...
		*/
		...
		$dates = ["trial_end" => WC_Subscriptions_Product::get_trial_expiration_date($product, $start_date), "next_payment" => WC_Subscriptions_Product::get_first_renewal_payment_date($product, $start_date), "end" => WC_Subscriptions_Product::get_expiration_date($product, $start_date), ];
	...
	function get_order_delivery_date_from_date($date = null, $group = null, $cap = null) {
		if (!$group && $cap) {
			$groups = get_posts(["post_type" => "delivery-group", "post_status" => "publish", "posts_per_page" => - 1, ]);
	...
	add_action("woocommerce_product_options_advanced", function () {
		woocommerce_wp_text_input(["id" => "_codice_confezionamento", "label" => "Codice Confezionamento", ]);
		woocommerce_wp_checkbox(["id" => "_is_magazzino", "label" => "È da Magazzino?", ]);
		woocommerce_wp_text_input(["id" => "_qty_acquisto", "label" => "Quantità (Acquisto)", ]);
		woocommerce_wp_text_input(["id" => "_uom_acquisto", "label" => "Cod. Unità di misura", ]);
	});
	add_action("woocommerce_product_options_general_product_data", function () {
		global $post;
		woocommerce_wp_text_input(["id" => "_prezzo_acquisto", "label" => "Prezzo di acquisto (€)", "placeholder" => "0.00", "description" => __("I valori decimali sono separati con un punto. Es. €2.30", "woocommerce"), ]);
		woocommerce_wp_checkbox(["id" => "_tipo_percentuale_ricarico", "label" => "Eredita percentuale ricarico dalla categoria", ]);
		woocommerce_wp_text_input(["id" => "_percentuale_ricarico", "label" => "Ricarico %", "placeholder" => "0", "description" => __("Valore della percentuale.", "woocommerce"), ]);
	});
	...
	add_action("rest_api_init", function () {
		register_rest_route("agrispesa/v1", "import-preferences", ["methods" => "POST", "permission_callback" => function () {
			return true;
		...
		register_rest_route("agrispesa/v1", "import-extra-preferences", ["methods" => "POST", "permission_callback" => function () {
			return true;
		}, "callback" => function ($request) {
			$lines = explode(PHP_EOL, $request->get_body());
			$users = [];
			...
			$categories = ["01" => "Verdura", "02" => "Frutta", "05" => "Formaggio", "06" => "Uova", "08" => "Carne", "10" => "Pesce", ];
			foreach ($employee_csv as $user) {
				$args = ["fields" => "ids", "meta_query" => [["key" => "navision_id", "value" => $user["idUtente"], "compare" => "=", ], ], ];
				$member_arr = get_users($args); //finds all users with this meta_key == 'member_id' and meta_value == $member_id passed in url
				if (empty($member_arr)) {
					continue;
				}
				...
				if (!empty($user["carni"])) {
					$hasCarne = false;
					$jsonPreferencesBlacklist[] = ["name" => "Carne", "substitute" => isset($categories[$user["carniSost"]]) ? $categories[$user["carniSost"]] : "-", ];
				}
				if (!empty($user["formaggi"])) {
					$hasFormaggi = false;
					$jsonPreferencesBlacklist[] = ["name" => "Formaggio", "substitute" => isset($categories[$user["formaggiSost"]]) ? $categories[$user["formaggiSost"]] : "-", ];
				}
				if (!empty($user["frutta"])) {
					$hasFrutta = false;
					$jsonPreferencesBlacklist[] = ["name" => "Frutta", "substitute" => isset($categories[$user["fruttaSost"]]) ? $categories[$user["fruttaSost"]] : "-", ];
				}
				if (!empty($user["pesce"])) {
					$hasPesce = false;
					$jsonPreferencesBlacklist[] = ["name" => "Pesce", "substitute" => isset($categories[$user["pesceSost"]]) ? $categories[$user["pesceSost"]] : "-", ];
				}
				if (!empty($user["uova"])) {
					$hasUova = false;
					$jsonPreferencesBlacklist[] = ["name" => "Uova", "substitute" => isset($categories[$user["uovaSost"]]) ? $categories[$user["uovaSost"]] : "-", ];
				}
				if (!empty($user["verdura"])) {
					$hasVerdura = false;
					$jsonPreferencesBlacklist[] = ["name" => "Verdura", "substitute" => isset($categories[$user["verduraSost"]]) ? $categories[$user["verduraSost"]] : "-", ];
				}
		...
		register_rest_route("agrispesa/v1", "import-subscriptions", ["methods" => "POST", "permission_callback" => function () {
			...
			foreach ($employee_csv as $user) {
				...
				$wordpressUser = get_users(["meta_key" => "navision_id", "meta_value" => $user["id_utente"], ]);
	...
	*/
			...
			$response = new WP_REST_Response(['subscriptions_created' => $subscriptionCreated, 'users_skipped' => $usersSkipped]);
	...
	WHERE post_type = 'product' AND ID NOT IN (" . implode(",", $idsToExclude) . ")");
		...
		register_rest_route("agrispesa/v1", "import-box", ["methods" => "POST", "permission_callback" => function () {
			...
			$allGroups = array_map(function($group){
				$group->products_sku = get_post_meta($group->ID,'products_sku',true);
				return $group;
			},$allGroups);
			...
			foreach($products["ROW"] as $product){
				...
				$isFoundGroup = array_filter($allGroups,function($group) use($sku){
					return is_array($group->products_sku) && in_array($sku,$group->products_sku);
				});
	...
	WHERE wp.ID IS NULL");
		...
		register_rest_route("agrispesa/v1", "import-invoices", ["methods" => "POST", "permission_callback" => function () {
			return true;
		}, "callback" => function ($request) {
			($xml = simplexml_load_string($request->get_body())) or die("Error: Cannot create object");
			$invoices = (array)$xml;
			...
			foreach ($invoices as $key => $invoice) {
				...
				$alreadyExists = get_posts(["post_type" => "invoice", "fields" => "ids", "post_status" => "publish", "posts_per_page" => 1, "meta_query" => [["key" => "_navision_id", "value" => $invoice["entry_id"], "compare" => "=", ], ], ]);
				if (!empty($alreadyExists)) {
					continue;
				}
				$my_post = ["post_title" => "Fattura #" . $invoice["entry_id"], "post_content" => "", "post_status" => "publish", "post_type" => "invoice", ];
			...
			update_option("last_import_invoices", (new DateTime())->format("Y-m-d H:i:s"));
		...
		register_rest_route("agrispesa/v1", "export-payments", ["methods" => "GET", "permission_callback" => function () {
			return true;
		}, "callback" => function ($request) {
		...
		register_rest_route("agrispesa/v1", "products/(?P<product_id>\d+)/category", ["methods" => "GET", "permission_callback" => function () {
			return true;
		}, "callback" => function ($request) {
		...
		register_rest_route("agrispesa/v1", "weekly-box", ["methods" => "POST", "permission_callback" => function () {
			return true;
		}, "callback" => function ($request) {
			$body = $request->get_json_params();
			$post_id = wp_insert_post(["post_type" => "weekly-box", "post_title" => "Box settimana " . $body["week"] . " - " . $body["product_box_id"], "post_content" => "", "post_status" => "publish", "comment_status" => "closed", // if you prefer
			"ping_status" => "closed", // if you prefer
			]);
			if ($post_id) {
				// insert post meta
		...
		register_rest_route("agrispesa/v1", "weekly-box/duplicate", ["methods" => "POST", "permission_callback" => function () {
			return true;
		}, "callback" => function ($request) {
			$body = $request->get_json_params();
			$lastWeek = $body["week"] - 1;
			$lastWeekBox = get_posts(["post_type" => "weekly-box", "post_status" => "publish", "posts_per_page" => 1, "meta_query" => ["relation" => "and", ["key" => "_week", "value" => str_pad($lastWeek, 2, 0, STR_PAD_LEFT), "compare" => "=", ], ["key" => "_product_box_id", "value" => $body["product_box_id"], "compare" => "=", ], ], ]);
			if (empty($lastWeekBox)) {
				$response = new WP_REST_Response(["message" => "Nessuna Box Settimana trovata per la settimana " . $lastWeek, ]);
		...
		register_rest_route("agrispesa/v1", "weekly-box/(?P<box_id>\d+)/products/(?P<index>\d+)", ["methods" => "DELETE", "permission_callback" => function () {
			return true;
		}, "callback" => function ($request) {
		...
		register_rest_route("agrispesa/v1", "weekly-box/(?P<box_id>\d+)/products", ["methods" => "POST", "permission_callback" => function () {
			return true;
		}, "callback" => function ($request) {
			$products = get_post_meta($request["box_id"], "_products", true);
			foreach ($request["product_ids"] as $key => $id) {
				...
				$products[] = ["id" => $id, "name" => $product->get_name(), "quantity" => $request["quantity"][$key], "price" => $product->get_price(), "unit_measure" => $unitaMisura, "unit_measure_print" => get_post_meta($id, "_uom_acquisto", true), ];
			}
		...
		register_rest_route("agrispesa/v1", "user-subscriptions", ["methods" => "GET", "permission_callback" => function () {
			return true;
		}, "callback" => function ($request) {
			$loggedUser = $_GET["userId"];
			$subscriptions = wcs_get_subscriptions(["subscriptions_per_page" => - 1, "customer_id" => $loggedUser, "subscription_status" => "active", ]);
			$json = [];
			foreach ($subscriptions as $subscription) {
				$products = $subscription->get_items();
				...
				$json[] = ["name" => reset($products)->get_name(), "id" => $subscription->get_id(), "box_preferences" => $boxPreferences, "box_blacklist" => $boxBlacklist, "products" => $productsToAdd, ];
			}
			$response = new WP_REST_Response($json);
			$response->set_status(200);
			return $response;
		}, ]);
		register_rest_route("agrispesa/v1", "delivery-group-csv", ["methods" => "GET", "permission_callback" => function () {
			return true;
		}, "callback" => function ($request) {
			$dataConsegna = $_GET["data_consegna"];
			$caps = get_post_meta($_GET["delivery_group"], "cap", true);
			$args = ["posts_per_page" => - 1, "post_type" => "shop_order", "post_status" => ["wc-processing", "wc-completed"], "meta_query" => ["relation" => "AND", ["key" => "_data_consegna", "value" => $dataConsegna, "compare" => "=", ], ["key" => "_shipping_postcode", "value" => $caps, "compare" => "IN", ], ], ];
			$orders = new WP_Query($args);
			$orders = $orders->get_posts();
			$csv = [];
			foreach ($orders as $order) {
				$order = wc_get_order($order->ID);
				$csv[] = [$order->get_shipping_postcode(), $order->get_shipping_city(), $order->get_shipping_address_1(), "", "", "", "", "", "", "", $order->get_shipping_first_name() . " " . $order->get_shipping_last_name(), ];
			}
		...
		register_rest_route("agrispesa/v1", "subscription-preference", ["methods" => "POST", "permission_callback" => function () {
			return true;
		}, "callback" => function ($request) {
			$body = $request->get_json_params();
		...
		register_rest_route("agrispesa/v1", "subscription-blacklist", ["methods" => "POST", "permission_callback" => function () {
			return true;
		}, "callback" => function ($request) {
			$body = $request->get_json_params();
		...
		register_rest_route("agrispesa/v1", "subscription-preference", ["methods" => "DELETE", "permission_callback" => function () {
			return true;
		}, "callback" => function ($request) {
			$body = $request->get_json_params();
			$boxPreferences = get_post_meta($body["subscription_id"], "_box_preferences", true);
		...
		register_rest_route("agrispesa/v1", "subscription-blacklist", ["methods" => "DELETE", "permission_callback" => function () {
			return true;
		}, "callback" => function ($request) {
			$body = $request->get_json_params();
			$boxPreferences = get_post_meta($body["subscription_id"], "_box_blacklist", true);
	...
	function my_enqueue($hook) {
		if ($hook == "edit.php" || $hook == "post.php") {
			wp_enqueue_script("agrispesa-admin-delivery-box-js", get_theme_file_uri("assets/js/admin-delivery-box.js"), ["jquery", "select2"], null, true);
			wp_localize_script("agrispesa-admin-delivery-box-js", "WPURL", ["siteurl" => get_option("siteurl"), "userId" => get_current_user_id(), ]);
		} else {
			if ("toplevel_page_esporta-documenti" == $hook) {
				wp_enqueue_script("agrispesa-export-js", get_theme_file_uri("assets/js/export.js"), ["jquery"], null, true);
				return;
			}
			...
			wp_register_script("select2", "//cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/js/select2.min.js", ["jquery"], "1.0", true);
			wp_enqueue_style("select2css");
			wp_enqueue_script("select2");
			wp_enqueue_script("moment", get_template_directory_uri() . "/assets/js/moment.min.js", ["jquery"], null, true);
			wp_register_script("axios", "//cdnjs.cloudflare.com/ajax/libs/axios/1.2.2/axios.min.js", [], null, true);
			wp_enqueue_script("axios");
			wp_register_script("vuejs", "//unpkg.com/vue@3/dist/vue.global.js", [], null, true);
			wp_enqueue_script("vuejs");
			wp_register_style("datatable", "//cdn.datatables.net/1.13.1/css/jquery.dataTables.min.css", false, "1.0", "all");
			wp_enqueue_style("datatable");
			wp_register_script("datatable-js", "//cdn.datatables.net/1.13.1/js/jquery.dataTables.min.js", [], null, true);
			wp_enqueue_script("datatable-js");
			wp_enqueue_style("agrispesa-admin-css", get_theme_file_uri("assets/css/admin.css"), false, "1.0", "all");
			wp_enqueue_script("agrispesa-admin-js", get_theme_file_uri("assets/js/admin.js"), ["jquery", "select2"], null, true);
			wp_localize_script("agrispesa-admin-js", "WPURL", ["siteurl" => get_option("siteurl"), "userId" => get_current_user_id(), ]);
		}
	}
	...
	if (!function_exists("mv_add_meta_boxes")) {
		function mv_add_meta_boxes() {
			add_meta_box("old_preferences", "Preferenze Utente Vecchio sito", "old_preferences_meta_box_callback", "shop_subscription", "normal", "default");
			add_meta_box("old_preferences_order", "Preferenze Utente Vecchio sito", "old_preferences_order_meta_box_callback", "shop_order", "advanced", "core");
			add_meta_box("box_preferences", "Preferenze Facciamo noi", "box_preferences_meta_box_callback", "shop_order", "advanced", "core", []);
		...
		function box_preferences_meta_box_callback($order) {
			...
			$boxPreferences = get_post_meta($subscriptionId, "_box_preferences", true);
	...
	*/
	?>
		<!--	<strong>Data di consegna:</strong><br>
			<select autocomplete="off" name="_data_consegna">
				...
				if (!$dataConsegna): ?>
						<option <?php if (!$consegna): ?> selected <?php
					endif; ?>>Nessuna data di consegna</option>
					<?php
				else: ?>
						<option
							<?php if ($consegna && $dataConsegna->format("Y - m - d") == $consegna): ?> selected <?php
					endif; ?>
	...
	function get_weekly_box_from_box($id, $week) {
		$box = get_posts(["post_type" => "weekly-box", "post_status" => "publish", "posts_per_page" => 1, "meta_query" => ["relation" => "and", ["key" => "_week", "value" => date("Y") . "_" . $week, "compare" => "=", ], ["key" => "_product_box_id", "value" => $id, "compare" => "=", ], ], ]);
		if (empty($box)) {
			return null;
		}
		return reset($box);
	...
	function generate_fabbisogno() {
		...
		$args = ["posts_per_page" => - 1, "post_type" => "shop_order", "post_status" => ["wc-processing", "wc-on-hold"], "meta_query" => ["relation" => "and", ["key" => "_week", "value" => $week, "compare" => "=", ], ], ];
		$orders = new WP_Query($args);
		$orders = $orders->get_posts();
		$fabbisogni = [];
		foreach ($orders as $order) {
			...
			foreach ($order->get_items() as $item) {
				...
				if (!isset($fabbisogni[$product->get_id() ])) {
					...
					$tmpFabbisogno = ["fabbisogno" => $quantity, "weight" => $weight, "product_name" => $product->get_name(), "quantity_type" => $measureAcquisto, "weight_type" => $measureUnit, "sku" => $product->get_sku(), "produttore" => $fornitoreString, "gruppo_consegna" => $gruppoConsegna, ];
	...
	WHERE wp.ID IS NULL
		...
		foreach ($fabbisogni as $key => $product) {
			...
			$post_id = wp_insert_post(["post_type" => "fabbisogno", "post_title" => $product["product_name"] . " - " . $gruppoConsegna, "post_content" => "", "post_status" => "publish", "comment_status" => "closed", // if you prefer
			"ping_status" => "closed", // if you prefer
			]);
	...
	*/
	...
	function get_all_single_box() {
		$products = get_posts(["post_type" => "product", "numberposts" => - 1, "post_status" => "publish", "tax_query" => [["taxonomy" => "product_cat", "field" => "slug", "terms" => "box singola", "operator" => "IN", ], ], ]);
		$singleBoxes = [];
		foreach ($products as $product) {
	...
	function get_subscription_box_from_attributes($tipologia, $dimensione) {
		$products = get_posts(["post_type" => "product", "numberposts" => - 1, "post_status" => "publish", "tax_query" => [["taxonomy" => "product_cat", "field" => "slug", "terms" => "box", "operator" => "IN", ], ], ]);
		$productFound = false;
		foreach ($products as $product) {
			$product = wc_get_product($product->ID);
			$children = $product->get_children();
	...
	function get_fabbisogno($week) {
		$fabbisognoList = new WP_Query(["posts_per_page" => - 1, "post_type" => "fabbisogno", "meta_query" => ["relation" => "and", ["key" => "settimana", "value" => $week, "compare" => "=", ], ], ]);
	...
	?>
		...
		<div id="wpbody-content">
			...
			$args = ["posts_per_page" => - 1, "post_type" => "shop_order", "post_status" => ["wc-processing", "wc-completed"], "meta_query" => ["relation" => "AND", ["key" => "_week", "value" => date("Y") . "_" . $week, "compare" => "=", ], ["key" => "_subscription_id", "value" => $subscription->get_id(), "compare" => "=", ], ], ];
			$orders = new WP_Query($args);
			$orders = $orders->get_posts();
			$products = $subscription->get_items();
	...
	function cptui_register_my_cpts_delivery_group() {
		 ...
		 * Post Type: Gruppi di Consegna.
		 */
		$labels = ["name" => esc_html__("Consegne", "custom-post-type-ui"), "singular_name" => esc_html__("Consegna", "custom-post-type-ui"), ];
		$args = ["label" => esc_html__("Consegna", "custom-post-type-ui"), "labels" => $labels, "description" => "", "public" => true, "publicly_queryable" => true, "show_ui" => true, "show_in_rest" => true, "rest_base" => "", "rest_controller_class" => "WP_REST_Posts_Controller", "rest_namespace" => "wp/v2", "has_archive" => false, "show_in_menu" => true, "show_in_nav_menus" => true, "delete_with_user" => false, "exclude_from_search" => false, "capability_type" => "post", "map_meta_cap" => true, "hierarchical" => false, "can_export" => false, "rewrite" => ["slug" => "delivery-item", "with_front" => true], "query_var" => true, "supports" => ["title", "editor"], "show_in_graphql" => false, ];
		$labels = ["name" => esc_html__("Box settimanali", "custom-post-type-ui"), "singular_name" => esc_html__("Box settimanale", "custom-post-type-ui"), ];
		$args = ["label" => esc_html__("Box settimanale", "custom-post-type-ui"), "labels" => $labels, "description" => "", "public" => false, "publicly_queryable" => false, "show_ui" => false, "show_in_rest" => true, "rest_base" => "", "rest_controller_class" => "WP_REST_Posts_Controller", "rest_namespace" => "wp/v2", "has_archive" => false, "show_in_menu" => false, "show_in_nav_menus" => false, "delete_with_user" => false, "exclude_from_search" => false, "capability_type" => "post", "map_meta_cap" => true, "hierarchical" => false, "can_export" => false, "rewrite" => ["slug" => "weekly-box", "with_front" => true], "query_var" => true, "supports" => ["title", "editor"], "show_in_graphql" => false, ];
		register_post_type("weekly-box", $args);
		$labels = ["name" => esc_html__("Fabbisogno", "custom-post-type-ui"), "singular_name" => esc_html__("Fabbisogno", "custom-post-type-ui"), ];
		$args = ["label" => esc_html__("Fabbisogno", "custom-post-type-ui"), "labels" => $labels, "public" => true, "publicly_queryable" => true, "show_ui" => true, "show_in_rest" => true, "rest_base" => "", "rest_controller_class" => "WP_REST_Posts_Controller", "rest_namespace" => "wp/v2", "has_archive" => false, "show_in_menu" => true, "show_in_nav_menus" => true, "delete_with_user" => false, "exclude_from_search" => false, "capability_type" => "post", "map_meta_cap" => true, "hierarchical" => false, "can_export" => false, "rewrite" => ["slug" => "fabbisogno", "with_front" => true], "query_var" => true, "supports" => ["title", "editor"], "show_in_graphql" => false, ];
			...
			if (isset($_POST["import_consegne"])) {
				if (isset($_FILES["file"])) {
					...
					if ($_FILES["file"]["error"] > 0) {
						...
						fclose($file);
						$args = ["posts_per_page" => - 1, "post_type" => "shop_order", "post_status" => ["wc-processing", "wc-completed"], "meta_query" => ["relation" => "AND", ["key" => "_data_consegna", "value" => $_POST["_data_consegna"], "compare" => "=", ], ], ];
						$orders = new WP_Query($args);
						$orders = $orders->get_posts();
						$i = 0;
	...
	?>
			...
			endforeach; ?>
				...
				$caps = get_post_meta($group->ID, "cap", true);
				$orders = wc_get_orders(["limit" => - 1, "meta_key" => "_gruppo_consegna", "meta_value" => $group->post_title, "meta_compare" => "=", ]);
	...
	?>
		...
		add_menu_page("Box Settimanali", "Box Settimanali", "manage_options", "box-settimanali", function () {
			...
			$boxs = get_posts(["post_type" => "weekly-box", "post_status" => "publish", "posts_per_page" => - 1, "orderby" => "meta_value_num", "meta_key" => "_week", "order" => "DESC", ]);
			$date = new DateTime();
			$currentWeek = $date->format("W");
			$products = get_posts(["post_type" => "product", "numberposts" => - 1, "post_status" => "publish", "tax_query" => [["taxonomy" => "product_cat", "field" => "slug", "terms" => "box singola", "operator" => "IN", ], ], ]);
			...
			$title = "";
			$empty = 0;
			$args = ["taxonomy" => $taxonomy, "orderby" => $orderby, "show_count" => $show_count, "pad_counts" => $pad_counts, "hierarchical" => $hierarchical, "title_li" => $title, "hide_empty" => $empty, ];
				...
				if ($cat->category_parent == 0) {
					...
					if ($sub_cats) {
						foreach ($sub_cats as $sub_category) {
							...
							$categories[$sub_category->term_id] = ["name" => $sub_category->name, "products" => $categoryProducts, ];
							foreach ($categoryProducts as $categoryProduct) {
								...
								$jsonProducts[] = ["id" => $categoryProduct->ID, "name" => $categoryProduct->post_title, "weight" => $weight, "sku" => get_post_meta($categoryProduct->ID, "_sku", true), "fornitore" => $fornitoreString, "unit_measure" => $unitaMisura, "codice_confezionamento" => get_post_meta($categoryProduct->ID, "_codice_confezionamento", true), "unit_measure_print" => get_post_meta($categoryProduct->ID, "_uom_acquisto", true), "price" => floatval($price), ];
							}
						}
					}
				}
	...
	?>
			...
			$loop_categories = get_categories(["taxonomy" => "product_cat", "hide_empty" => 1, "parent" => $negozioID, ]);
			foreach ($loop_categories as $loop_category) {
				$args = ["posts_per_page" => - 1, "tax_query" => ["relation" => "AND", "hide_empty" => 1, "paged" => false, ["taxonomy" => "product_cat", "field" => "slug", "terms" => $loop_category->slug, ], ], "post_type" => "product", "orderby" => "menu_order", "order" => "asc", "meta_query" => [["key" => "_is_active_shop", "value" => "1", "compare" => "==", ], ], ];
	...
	?>
			...
			$getIDbyNAME = get_term_by("name", "negozio", "product_cat");
			$negozioID = $getIDbyNAME->term_id;
			$loop_categories = get_categories(["taxonomy" => "product_cat",
			// 'orderby' => $orderby,
			// 'meta_key' => $meta_key,
			"hide_empty" => 1, "parent" => $negozioID, ]);
			foreach ($loop_categories as $loop_category) {
				$args = ["posts_per_page" => - 1, "tax_query" => ["relation" => "AND", "hide_empty" => 1, "paged" => false, ["taxonomy" => "product_cat", "field" => "slug", "terms" => $loop_category->slug, ], ], "post_type" => "product", "orderby" => "menu_order", "order" => "asc", "meta_query" => [["key" => "_is_active_shop", "value" => "1", "compare" => "!=", ], ], ];
				$cat_query = new WP_Query($args);
				$count_posts = new WP_Query($args);
	...
	?>
				...
				echo '<span style="display:block;">' . date_format($boxdate, "Y/m/d") . '</span><span style="display:block;margin-top:4px;">' . date_format($boxdate, "H:i:s") . "</span>";
	?></span>
									</td>
									<td class="author column-author" data-colname="Autore" style="padding:25px 10px 10px;">
												...
												<tr class="create-box-table--row">
													...
													<td class="create-box-table--quantity">
														<input style="width:70px;" readonly
															...
															<?php if ($week < $currentWeek): ?> disabled <?php
					endif; ?>
															   type="number"
	...
	?>
												...
												<?php if (is_array($dataConsegna["meta_value"]) || empty($dataConsegna["meta_value"])) {
						continue;
					} ?>
	...
	?>
										<select autocomplete="off" name="settimana">
											...
											<?php foreach ($allSettimaneFabbisogno as $week): ?>
												<option
													<?php if ($week == $currentWeek): ?> selected <?php
				endif; ?>
	...
	function variation_settings_fields($loop, $variation_data, $variation) {
		...
		woocommerce_wp_text_input(["id" => "_navision_id{$loop}", "name" => "_navision_id[{$loop}][]", "wrapper_class" => "form-row form-row-full", "label" => "ID Navision", "value" => $idNavision, ], $variation_data->ID);
	}
