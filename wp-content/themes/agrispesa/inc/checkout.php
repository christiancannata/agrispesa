...
//add_filter( 'manage_edit-shop_order_columns', 'custom_shop_order_column_nathi', 20 );
function custom_shop_order_column_nathi($columns)
{
...
//add_action( 'manage_shop_order_posts_custom_column' , 'custom_orders_list_column_content_nathi', 20, 2 );
function custom_orders_list_column_content_nathi( $column, $post_id )
{
    switch ( $column )
    {
        case 'my-column1' :
            // Get custom post meta data
            $my_var_one = get_post_meta( $post_id, '_order_type', true );
            if(!empty($my_var_one))
                echo $my_var_one;
