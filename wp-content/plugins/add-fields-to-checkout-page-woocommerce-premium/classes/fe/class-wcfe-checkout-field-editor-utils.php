...
abstract class WCFE_Checkout_Fields_Utils {
	...
	public function woo_email_customer_details($order, $sent_to_admin, $plain_text, $email){
		$settings = $this->get_advanced_settings();
	...
	public function woo_email_customer_details_fields($ofields, $sent_to_admin, $order){
		$settings = $this->get_advanced_settings();
		$position = $this->get_setting_value($settings, 'custom_fields_position_email');
		$html_enabled = $this->get_setting_value($settings, 'enable_html_in_emails') === 'yes' ? true : false;
	...
	public function woo_email_order_meta($order, $sent_to_admin, $plain_text, $email){
		$settings = $this->get_advanced_settings();
		$position = $this->get_setting_value($settings, 'custom_fields_position_email');
		$html_enabled = $this->get_setting_value($settings, 'enable_html_in_emails') === 'yes' ? true : false;
	...
	public function woo_email_order_meta_fields($ofields, $sent_to_admin, $order){
		$settings = $this->get_advanced_settings();
		$position = $this->get_setting_value($settings, 'custom_fields_position_email');
		$html_enabled = $this->get_setting_value($settings, 'enable_html_in_emails') === 'yes' ? true : false;
	...
	public function get_custom_fields_for_display_in_emails($order, $sent_to_admin){
		...
		$custom_fields = array();
		foreach($sections as $sname => $section){
			$sfields = $this->get_section_fields_for_display_in_emails($order_id, $section, $sent_to_admin, $rfnames, $is_nl2br);
			if(is_array($sfields)){
				$custom_fields = array_merge($custom_fields, $sfields);
			}

			if($sname != 'billing' && $sname != 'shipping' && $sname != 'additional'){
				$rsfields = $this->get_repeat_section_fields_for_display_in_emails($order_id, $sname, $section, $sent_to_admin, $rsnames, $rfnames, $is_nl2br);
	...
	public function get_repeat_section_fields_for_display_in_emails($order_id, $key, $section, $sent_to_admin, $rsnames, $rfnames, $is_nl2br=true){
		$custom_fields = array();
		...
		if(is_array($rsections)){
			foreach($rsections as $rskey => $rsection) {
				$fields = $this->get_section_fields_for_display_in_emails($order_id, $rsection, $sent_to_admin, $rfnames, $is_nl2br);
				if(is_array($fields)){
	...
	public function get_section_fields_for_display_in_emails($order_id, $section, $sent_to_admin, $rfnames, $is_nl2br=true){
		$custom_fields = array();
	...
	public static function get_file_display_name($upload_info, $downloadable=true){
		...
		if(is_array($upload_info)){
			if( is_array( $upload_info['name'] ) ) {
				$multiName = array();
				foreach ($upload_info['name'] as $infokey => $infovalue) {
					$dname = isset($upload_info['name'][$infokey]) ? $upload_info['name'][$infokey] : '';
					$url = isset($upload_info['url'][$infokey]) ? $upload_info['url'][$infokey] : '';
					$price_info = isset($upload_info['price_info'][$infokey]) ? $upload_info['price_info'][$infokey] : '';
				...
				$string_array = implode("<br>",$multiName);
	...
	public static function bad_json_formator($match){
	    $key = trim($match[1]);
	    $val = trim($match[2]);
	...
	public static function get_file_display_name_order($upload_info_json, $downloadable=true){
		...
		if($upload_info_json){
			...
			$preg = preg_replace_callback("#([^{:]*):([^,}]*)#i",'self::bad_json_formator',$upload_info_json);

			$upload_info = json_decode($preg, true);
