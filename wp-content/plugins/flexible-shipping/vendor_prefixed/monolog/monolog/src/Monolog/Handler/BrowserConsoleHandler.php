...
class BrowserConsoleHandler extends \FSVendor\Monolog\Handler\AbstractProcessingHandler
{
    ...
    {
        return [\FSVendor\Monolog\Logger::DEBUG => 'debug', \FSVendor\Monolog\Logger::INFO => 'info', \FSVendor\Monolog\Logger::NOTICE => 'info', \FSVendor\Monolog\Logger::WARNING => 'warn', \FSVendor\Monolog\Logger::ERROR => 'error', \FSVendor\Monolog\Logger::CRITICAL => 'error', \FSVendor\Monolog\Logger::ALERT => 'error', \FSVendor\Monolog\Logger::EMERGENCY => 'error'][$level] ?? 'log';
    }
    /**
     * @return string[]
     */
    private static function handleStyles(string $formatted) : array
    {
        ...
        foreach (\array_reverse($matches) as $match) {
            ...
            $pos = $match[0][1];
            $format = \FSVendor\Monolog\Utils::substr($format, 0, $pos) . '%c' . $match[1][0] . '%c' . \FSVendor\Monolog\Utils::substr($format, $pos + \strlen($match[0][0]));
        }
    ...
    private static function handleCustomStyles(string $style, string $string) : string
    {
        ...
        if (null === $style) {
            ...
            throw new \RuntimeException('Failed to run preg_replace_callback: ' . $pcreErrorCode . ' / ' . \FSVendor\Monolog\Utils::pcreLastErrorMessage($pcreErrorCode));
        }
        return $style;
    }
    /**
