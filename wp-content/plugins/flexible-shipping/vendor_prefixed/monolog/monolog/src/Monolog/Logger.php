...
class Logger implements \Psr\Log\LoggerInterface, \FSVendor\Monolog\ResettableInterface
{
     ...
     */
    public function pushHandler(\FSVendor\Monolog\Handler\HandlerInterface $handler) : self
    {
     ...
     */
    public function popHandler() : \FSVendor\Monolog\Handler\HandlerInterface
    {
        if (!$this->handlers) {
     ...
     */
    public function addRecord(int $level, string $message, array $context = [], \FSVendor\Monolog\DateTimeImmutable $datetime = null) : bool
    {
        ...
        try {
            ...
            foreach ($this->handlers as $handler) {
                if (null === $record) {
                    ...
                    $record = ['message' => $message, 'context' => $context, 'level' => $level, 'level_name' => $levelName, 'channel' => $this->name, 'datetime' => $datetime ?? new \FSVendor\Monolog\DateTimeImmutable($this->microsecondTimestamps, $this->timezone), 'extra' => []];
                    try {
