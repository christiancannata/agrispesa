...
class ChromePHPHandler extends \FSVendor\Monolog\Handler\AbstractProcessingHandler
{
    ...
    protected function send() : void
    {
        ...
        $json = \FSVendor\Monolog\Utils::jsonEncode(self::$json, \FSVendor\Monolog\Utils::DEFAULT_JSON_FLAGS & ~\JSON_UNESCAPED_UNICODE, \true);
        $data = \base64_encode($json);
        if (\strlen($data) > 3 * 1024) {
            self::$overflowed = \true;
            $record = ['message' => 'Incomplete logs, chrome header size limit reached', 'context' => [], 'level' => \FSVendor\Monolog\Logger::WARNING, 'level_name' => \FSVendor\Monolog\Logger::getLevelName(\FSVendor\Monolog\Logger::WARNING), 'channel' => 'monolog', 'datetime' => new \DateTimeImmutable(), 'extra' => []];
