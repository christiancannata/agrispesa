...
class SyslogUdpHandler extends \FSVendor\Monolog\Handler\AbstractSyslogHandler
{
     ...
     */
    public function __construct(string $host, int $port = 514, $facility = \LOG_USER, $level = \FSVendor\Monolog\Logger::DEBUG, bool $bubble = \true, string $ident = 'php', int $rfc = self::RFC5424)
    {
        if (!\extension_loaded('sockets')) {
            throw new \FSVendor\Monolog\Handler\MissingExtensionException('The sockets extension is required to use the SyslogUdpHandler');
        }
        parent::__construct($facility, $level, $bubble);
        $this->ident = $ident;
        $this->rfc = $rfc;
        $this->socket = new \FSVendor\Monolog\Handler\SyslogUdp\UdpSocket($host, $port);
    }
    ...
    private function splitMessageIntoLines($message) : array
    {
        ...
        if (\false === $lines) {
            ...
            throw new \RuntimeException('Could not preg_split: ' . $pcreErrorCode . ' / ' . \FSVendor\Monolog\Utils::pcreLastErrorMessage($pcreErrorCode));
        }
        return $lines;
    }
    /**
     ...
     */
    public function setSocket(\FSVendor\Monolog\Handler\SyslogUdp\UdpSocket $socket) : self
    {
        $this->socket = $socket;
        return $this;
    }
