...
class GelfMessageFormatter extends \FSVendor\Monolog\Formatter\NormalizerFormatter
{
     ...
     */
    private $logLevels = [\FSVendor\Monolog\Logger::DEBUG => 7, \FSVendor\Monolog\Logger::INFO => 6, \FSVendor\Monolog\Logger::NOTICE => 5, \FSVendor\Monolog\Logger::WARNING => 4, \FSVendor\Monolog\Logger::ERROR => 3, \FSVendor\Monolog\Logger::CRITICAL => 2, \FSVendor\Monolog\Logger::ALERT => 1, \FSVendor\Monolog\Logger::EMERGENCY => 0];
    public function __construct(?string $systemName = null, ?string $extraPrefix = null, string $contextPrefix = 'ctxt_', ?int $maxLength = null)
    {
        if (!\class_exists(\FSVendor\Gelf\Message::class)) {
        ...
        if (\method_exists(\FSVendor\Gelf\Message::class, 'setFacility')) {
            $this->gelfVersion = 1;
        }
    }
    /**
     * {@inheritDoc}
     */
    public function format(array $record) : \FSVendor\Gelf\Message
    {
        $context = $extra = [];
        ...
        if ($len > $this->maxLength) {
            $message->setShortMessage(\FSVendor\Monolog\Utils::substr($record['message'], 0, $this->maxLength));
        }
        ...
        foreach ($extra as $key => $val) {
            ...
            if ($len > $this->maxLength) {
                $message->setAdditional($this->extraPrefix . $key, \FSVendor\Monolog\Utils::substr((string) $val, 0, $this->maxLength));
                continue;
            }
        ...
        foreach ($context as $key => $val) {
            ...
            if ($len > $this->maxLength) {
                $message->setAdditional($this->contextPrefix . $key, \FSVendor\Monolog\Utils::substr((string) $val, 0, $this->maxLength));
                continue;
            }
