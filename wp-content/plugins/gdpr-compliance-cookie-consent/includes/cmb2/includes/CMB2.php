...
class CMB2 extends CMB2_Base {
    ...
    public function render_group_callback( $field_args, $field_group ) {
        ...
        foreach ( $group_val as $field_id ) {
            echo '<a href="#" id="'.$field_group->id().'" class="cmb-group-title" data-href="'.$field_group->id().'">', $field_group->replace_hash( $field_group->options( 'group_title' ) ), '</a>';
        }

        echo '<div id="'.$field_group->id().'" class="cmb-row cmb-repeat-group-wrap ', esc_attr( $field_group->row_classes() ), '" data-fieldtype="group"><div class="cmb-td"><div data-groupid="', esc_attr( $field_group->id() ), '" id="', esc_attr( $field_group->id() ), '_repeat" ', $this->group_wrap_attributes( $field_group ), '>';
        ...
        echo '
		<p class="submit custom"><input type="submit" name="submit-cmb" id="submit-cmb" class="button button-primary" value="Save Changes"></p>
		</div></div></div>';
