jQuery(function ($) {
    ...
    if (typeof wc_country_select_params === 'undefined' || typeof italy_city_and_postcode_select_params === 'undefined') {
        return false;
    }
            ...
            return {
                'language': {
                    errorLoading: function () {
                        // Soluzione https://github.com/select2/select2/issues/4355 riguardo i18n_ajax_error.
            ...
            $('select.city_select:visible').each(function () {
                ...
                $(this)
                    .on('select2:select', function () {
                        $(this).focus(); // Mantieni il focus dopo il select https://github.com/select2/select2/issues/4384
                    })
    ...
    $(document.body).on('state_changing', function (e, country, state, $container) {
        ...
        if (italy_city_and_postcode_select_params.cities[country]) {
            /* Se la nazione non ha stati/province */
            if (state) {
                if (italy_city_and_postcode_select_params.cities[country][state]) {
                    cityToSelect($citybox, italy_city_and_postcode_select_params.cities[country][state]);
                } else {
                    cityToInput($citybox);
                }
            } else {
    ...
    /* Ajax rimpiazza .cart_totals (figlio di .cart-collaterals) in shipping calculator */
    ...
    function cityToSelect($citybox, current_cities) {
        ...
        var $defaultOption = $('<option></option>')
            .attr('value', '')
            .text(italy_city_and_postcode_select_params.i18n_select_city_text);
        ...
        for (var index in current_cities) {
            if (current_cities.hasOwnProperty(index)) {
                ...
                if (current_cities[index] instanceof Array) {
					//if (current_cities[index][1]){
						
				 //if (current_cities[index] instanceof Array && typeof current_cities[index][1] !== 'undefined') {
				//if (current_cities[index] instanceof Array) {
                    var cityName = current_cities[index][0];
                    //$option.attr('data-postcode', current_cities[index][1]!== undefined); darà vero o falso
