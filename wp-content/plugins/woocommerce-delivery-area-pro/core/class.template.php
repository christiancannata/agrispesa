	...
	class FlipperCode_HTML_Markup {
			...
			foreach ( $templateCategories as $templateType ) {
				if($accordian == true) {
					...
					$html .= FlipperCode_HTML_Markup::get_element_html(
						$templateType . '_group', 'group', array(
							'value' => $atts['lable'],
				...
				foreach ( $directories as $key => $directory ) {
					...
					$user_uploaded_template = array_filter($fromUploads, function($var) use ($directory){
		...
		public static function field_templates( $name, $atts ) {
			...
			if(isset($atts['enable_slider']) && !empty($atts['enable_slider']) && $atts['enable_slider'] == true) {
				$templatesHtml .= FlipperCode_HTML_Markup::get_all_templates_with_slider( $name, $atts, $accordian );
			}else{
				$templatesHtml .= FlipperCode_HTML_Markup::get_all_templates( $name, $atts, $accordian );
			}
					...
					case 'post_type':
						...
						$posts = get_posts(
							...
							apply_filters(
								...
								'fc_control_post_query_args', array(
