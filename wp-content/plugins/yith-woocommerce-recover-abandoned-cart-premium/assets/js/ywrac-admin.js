(function($){
   ...
   $('#ywrac_user_guest_enabled-privacy').change();
     ...
     */
    $(document).on('change', '.ywrac-toggle-enabled input', function () {
        var enabled = $(this).val() === 'yes' ? 'yes' : 'no',
            container = $(this).closest('.ywrac-toggle-enabled'),
            email_template_id = container.data('email-template-id'),
            security = container.data('security');

        $.ajax({
            type: 'POST',
            data: {
                action: 'email_template_toggle_enabled',
                id: email_template_id,
                enabled: enabled,
                security: security
            },
            url: yith_ywrac_admin.ajaxurl,
