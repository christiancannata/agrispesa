...
wc_print_notices();
$customer = isset( $_GET['customer'] ) ? sanitize_text_field( wp_unslash( $_GET['customer'] ) ) : ''; //phpcs:ignore
?>
<div>
	<p><?php printf( esc_html( __( 'If you don\'t want to receive any more abandoned cart reminders, please retype your email address: %s', 'yith-woocommerce-recover-abandoned-cart' ) ), '<b>' . esc_html( urldecode( $customer ) ) . '</b>' ); //phpcs:ignore ?></p>
	...
	<p class="form-row form-row-wide">
		...
		<input type="hidden" name="email_hash" id="email_hash" value="<?php echo esc_attr( $customer ); ?>" />
