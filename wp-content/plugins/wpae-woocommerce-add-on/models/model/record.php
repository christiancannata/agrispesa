 ...
 */
class PMWE_Model_Record extends PMXE_Model {
	/**
	 * Initialize model
	...
	/**
	 * @see PMWE_Model::getBy()
	 * @return PMWE_Model_Record
	 */
	 ...
	 * @return PMWE_Model_List
	 */
	public function getRelated($model, $keyAssoc = NULL) {
		$related = new $model();
		if ( ! empty($this->id)) {
			if (is_null($keyAssoc)) {
				$defaultPrefix = strtolower(preg_replace('%^' . strtoupper(PMWE_Plugin::PREFIX) . '|_Record$%', '', get_class($this)));
				$keyAssoc = array();
		...
		return $related instanceof PMWE_Model_List ? $related->convertRecords() : $related;
	}
